...
try:
    # This line attempts to initialize the camera. If no camera is found
    # or it's not accessible, Picamera2 can raise an IndexError or RuntimeError.
    camera = Picamera2()
except IndexError:
    print("Error: No camera detected or accessible. Please check camera connection and `raspi-config` settings.")
    print("Ensure the camera module is enabled and properly connected.")
    sys.exit(1) # Exit the script if no camera is found
except RuntimeError as e:
    print(f"Error initializing Picamera2: {e}")
    print("This might indicate an issue with the camera module, its firmware, or libcamera setup.")
    sys.exit(1) # Exit the script for other runtime errors
